...
This ECIP defines the Ethereum Classic Funding Proposal (ECFP) process: a standardized, permissionless mechanism for submitting and executing funding requests from the Olympia Treasury (ECIP-1112), governed by the Olympia DAO (ECIP-1113). The ECFP format provides a reproducible structure for transparent and accountable allocation of protocol-level funds. It enables developers, infrastructure operators, researchers, and ecosystem contributors to submit auditable proposals for on-chain funding. By formalizing a reproducible structure for submission, voting, and disbursement, this process ensures treasury allocations are accountable, equitable, and aligned with Ethereum Classic’s principles of decentralization and sustainability.

### Abstract

This ECIP formalizes the Ethereum Classic Funding Proposal (ECFP) process as the standard submission, voting, and execution mechanism for allocating funds from the Olympia Treasury (ECIP-1112), governed by the Olympia DAO (ECIP-1113). The ECFP mechanism provides a transparent, reproducible pathway for protocol-level funding, enabling contributors to propose work that aligns with the network’s values and strategic priorities.
...
Historically, Ethereum Classic has depended on centralized foundations and private donors to fund protocol development, often through opaque processes with limited community oversight. This model introduced risks of gatekeeping, funding inconsistency, and strategic misalignment. Critical public goods — such as client maintenance, infrastructure, documentation, security audits, and educational resources — were funded in an ad hoc manner, leading to inconsistent support, limited accountability, and community mistrust.

With the activation of the Olympia Treasury (ECIP-1112) and the governance system established by the Olympia DAO (ECIP-1113), Ethereum Classic now has a decentralized, protocol-native funding source. To ensure these funds are allocated transparently and without reliance on trusted intermediaries, a standardized, auditable framework is required for submitting, evaluating, and executing funding proposals.
...
- **Improving accountability** — all proposal data and voting outcomes are recorded on-chain and auditable.
...
4. **Execution and Disbursement**
   ...
   - Treasury disbursements SHALL use deterministic proposal hashes for release validation, binding off-chain metadata (e.g., IPFS or Arweave CID) to an immutable execution payload.
...
To maintain resilience, openness, and resistance to manipulation, the following mitigations MAY be enforced by Olympia DAO:

- **Participation Thresholds**  
  Submission MAY require:
  - Token staking or refundable deposit (e.g., returned if quorum met)
  - Minimal ETC or governance token holding
  - Optional integration with Sybil-resistance systems such as BrightID, Gitcoin Passport, or Proof of Humanity. These integrations are permissionless, modular, and opt-in — enabling proposal authors or sub-DAOs to define eligibility rules based on external identity attestations.

- **Submission Cooldowns**  
  - Rate limits MAY be applied to deter flooding or proposal spamming.

- **Draft Review Periods**  
  - Proposals MUST remain in `Draft` status for a minimum review period before activation, to allow for community feedback and informal discussion.
...
- At launch, Olympia DAO uses a tokenless, one-address-one-vote model as defined in [ECIP-1113](https://ecips.ethereumclassic.org/ECIPs/ecip-1113).
- The DAO MAY, through on-chain governance, upgrade to other voting systems — including token-based (e.g., stake-weighted or delegated stake), quadratic, identity-verified, soulbound token, or hybrid models.
- Any such change MUST be proposed and approved via the standard ECFP governance process and adhere to quorum, threshold, and timelock requirements.

#### Child-DAO ECFPs for Infrastructure Spending

To streamline recurring, non-optional funding (e.g., client maintenance, RPC services), the DAO MAY define **Child-DAO ECFP categories** with specialized workflows and delegated budgets, consistent with the governance framework in [ECIP-1113](https://ecips.ethereumclassic.org/ECIPs/ecip-1113).
...
- May be sunset, reauthorized, or have its budget adjusted through ECFP proposals.
...
- All disbursements from the Olympia Treasury MUST still pass through main DAO governance via structured ECFPs or scheduled approvals.
...
- The ECFP process is the **exclusive, on-chain mechanism** for all Olympia Treasury disbursements, including:
  - Core protocol development
  - Emergency funding
  - Recurring or operational budgets
  - Multi-phase or milestone-based grants

- The Olympia DAO contract, as defined in [ECIP-1113](https://ecips.ethereumclassic.org/ECIPs/ecip-1113), SHALL be the sole authorized caller of the `release()` and `execute()` functions on the Olympia Treasury (ECIP-1112).
- All fund movements from the Treasury MUST be governed exclusively by successful DAO proposals following the ECFP process and enforced in the treasury contract logic.
- Treasury access is immutable, governance-isolated, and cannot be overridden by any off-chain or discretionary process.
...
- The Ethereum Classic DAO LLC SHALL operate solely as a legal execution proxy for approved on-chain proposals. It SHALL perform no evaluative, filtering, or discretionary role, and SHALL be fully subordinate to the on-chain governance outcome.
...
- The LLC’s scope and actions SHALL be publicly verifiable, auditable, and bound by a limited administrative mandate.

#### Donation Handling and Segregation

The Ethereum Classic DAO LLC MAY receive off-chain donations in fiat or crypto. All such donations:

- MUST be:
  - Tracked separately from treasury funds  
  - Routed through on-chain mechanisms when feasible  
...
- Use of donor funds outside the ECFP process MUST be:
  ...
  - Strictly separated from protocol-level treasury disbursements

- Donor funds MUST NOT override or bypass DAO-approved allocations, and off-chain spending MAY NOT compromise governance integrity. All donation handling SHALL be publicly verifiable and governed by a limited administrative mandate.
...
In addition to individual contributors, the ECFP framework — working in coordination with the Ethereum Classic DAO LLC — enables transparent and accountable participation by centralized or institutional donors, such as:
...
- **Ronin DAO** — Treasury-funded infrastructure and ecosystem support.
- **Mantle Governance** — Modular treasury management with delegated authority.
...
Proposal metadata MUST be hashed and linked to content-addressed storage (e.g., IPFS or Arweave) using the following structure, ensuring that off-chain proposal content is immutably bound to the on-chain execution payload for auditability:
...
- The dashboard MUST be open-source and verifiably mirror on-chain state, leveraging standardized on-chain events for proposal lifecycle and execution tracking.
...
- **Client Compatibility**:  
  Nodes that implement ECIP-1111 through ECIP-1114 will remain in consensus. Clients that do not support ECIP-1111 will fork from the canonical chain at the designated activation block. Clients that support ECIP-1111 and ECIP-1112 but not ECIP-1113 or ECIP-1114 will remain in consensus but will be unable to process governance transactions or authorize treasury disbursements.
...
The proposal hash serves as the unique identifier for execution. It links the on-chain disbursement to an immutable, off-chain metadata record stored on IPFS or Arweave, guarding against tampering and ensuring verifiability of funding intent and deliverables.

All ECFPs MUST:
- Embed a verifiable content hash (e.g., IPFS CID or Arweave digest) in the on-chain metadata to ensure tamper resistance.
...
- Include the chain ID and a domain separator in the hash to prevent replay attacks across testnets or forks.
...
- **Immutable Lifecycle States**:  
  Once an ECFP enters `Active` state (under vote), it becomes immutable. No edits or withdrawals are permitted, to preserve voting integrity.

- **Redundant Event Logging**:  
  Both DAO and Treasury contracts SHOULD emit standard events (e.g., `ProposalSubmitted`, `ProposalExecuted`) for independent indexing and traceability.
...
- **Proposal Manipulation**:  
  ...
  - Mandatory review periods before activation  
  - Metadata hash enforcement at execution time
...
- **Sybil Attack Mitigation**:  
  ...
  - Contributor identity attestation or whitelisting (permissionless, modular, and opt-in)
...
- Full on-chain transparency of proposal lifecycle, voting records, and disbursements
...
These precedents support the security, sustainability, and feasibility of the Ethereum Classic Funding Proposal (ECFP) system. ECIP-1114 builds on their lessons while tailoring the process to Ethereum Classic's values of decentralization, transparency, immutability, and long-term sustainability.
