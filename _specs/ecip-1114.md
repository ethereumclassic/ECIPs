...
To defend the ECFP system against spam, griefing, and plutocratic dominance, the following mitigations MAY be implemented by Olympia DAO:

- **Minimal Participation Thresholds**:  
  Proposal submission MAY require:
  - Holding a small amount of ETC or governance token  
  - Staking or gas deposits (refundable if quorum is met)

- **Cooldown Periods & Rate Limits**:  
  Limits on simultaneous active proposals or enforced cooldowns between submissions help prevent denial-of-service vectors.

- **Mandatory Review Periods**:  
  Every ECFP enters a `Draft` period before voting, allowing the community to validate proposal format, scope, and deliverables.

- **Structured Proposal Requirements**:  
  Standardized templates, metadata hashes (e.g., IPFS CIDs), and clear KPIs raise the minimum effort for spam submissions.

- **DAO Upgrade Paths for Voting Models**:  
  While token-weighted voting is the initial model, the DAO MAY upgrade to more inclusive models (e.g., quadratic voting, reputation scoring) via governance proposals.

These measures ensure the ECFP process remains permissionless while resilient to abuse.
...
#### Built for Present and Future Use

The ECFP framework is a **minimal set of rules** to ensure transparency and accountability:

- Proposals can be as simple as a one-time payout or as advanced as milestone-based vesting.
- The system imposes no minimum activity levels — it functions even if only one ECFP is approved per year.
- As ETC’s community grows, the same framework scales effortlessly.

This avoids complexity while ensuring ETC is never again dependent on private foundations or informal grants.
