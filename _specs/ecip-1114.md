...
This ECIP defines the Ethereum Classic Funding Proposal (ECFP) process: a standardized, permissionless mechanism for requesting protocol-level funding from the Olympia Treasury, governed by the Olympia DAO. The ECFP format enables developers, infrastructure operators, researchers, and ecosystem contributors to submit transparent, auditable proposals for on-chain funding. By formalizing a reproducible structure for submission, voting, and disbursement, this process ensures treasury allocations are accountable, equitable, and aligned with Ethereum Classic’s principles of decentralization and sustainability.

### Abstract

This ECIP defines the Ethereum Classic Funding Proposal (ECFP) process as the standard format and lifecycle for submitting and executing funding requests to the Olympia Treasury, governed by the Olympia DAO. The ECFP mechanism provides a transparent, reproducible pathway for protocol-level funding, enabling contributors to propose work that aligns with the network’s values and strategic priorities.
...
- **Preserving institutional knowledge** — approved and rejected proposals remain publicly archived for future reference.
...
2. **Proposal Statuses**
   ...
   - `Expired`: Lapsed without vote within the valid period.
...
- **Structured Format Requirements**  
  - Submissions MUST follow approved templates and include content-addressed metadata (e.g., IPFS CID).
...
- The LLC SHALL act solely as an executor for approved DAO proposals and SHALL NOT make independent decisions.
...
**Non-normative best practice**: The DAO MAY offer a “Donation ECFP” format allowing donors to suggest proposal-aligned funding streams routed through on-chain governance.
...
- **Upgradeable** — The process is governance-agnostic and supports future upgrades to DAO logic or voting models.
