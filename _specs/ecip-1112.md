...
#### Protection Against Unauthorized Access and Spam

The Olympia Treasury contract is hardened against unauthorized disbursement attempts:

- **DAO-Only Execution**:  
  Only the authorized DAO executor contract MAY trigger `release()` or `execute()` functions. Calls from EOAs or unauthorized contracts are rejected.

- **No Proposal Logic in Treasury**:  
  The treasury contract performs no evaluation of proposals and contains no governance logic, reducing its surface area for spam or attack vectors.

- **Rate-Limited by DAO Governance**:  
  Proposal throughput is rate-limited at the governance contract layer, protecting the treasury from excessive or malicious execution attempts.

These constraints ensure the treasury acts as a secure, passive vault controlled solely through verified governance channels.

### Rationale

#### Right-Sized for Network Maturity

The Olympia Treasury is deployed as a **simple, immutable vault**, with no internal governance or complex logic. Until the DAO activates, it accumulates funds securely with **no risk of misuse**.

This ensures:
- **No operational risk** while governance matures  
- **No funding loss or overhead** from inactive systems  
- **A secure foundation** for future coordination, without forcing premature adoption

Treasury logic is intentionally minimal. Complexity, if needed, is pushed to the DAO layer — which can evolve at the community’s pace.
...
The Olympia Treasury is a standalone, immutable smart contract that may be deployed and activated before the DAO (ECIP-1113) or legal execution interface (ECIP-1114) are finalized. Once deployed, the treasury can receive BASEFEE funds redirected by ECIP-1111 without requiring immediate disbursement capability.

This design allows funds to accumulate securely on-chain while governance contracts are tested and deployed. Until the DAO is active, the treasury contract will reject any withdrawal attempts, ensuring security and forward compatibility. This decouples treasury infrastructure from governance infrastructure, supporting a phased rollout of the Olympia Upgrade.
...
The Olympia Treasury contract must adhere to strict security guarantees to ensure integrity, immutability, and protocol safety. Security measures are grouped by concern below:

---

#### 1. Contract Immutability and Access Control

- **No Upgradability or Admin Keys**:  
  The Treasury contract is deployed as **immutable code** with **no proxy**, **no delegatecall**, and **no admin modifiers**. No entity — including the DAO — can modify the contract after deployment.

- **Access Control**:  
  Only the authorized **DAO executor contract** can call `release()` or `execute()`.  
  Any calls from EOAs or unauthorized contracts will **revert automatically**.

- **Minimal Attack Surface**:  
  Treasury logic is **intentionally minimal**. It performs only basic permission checks and transfers approved funds — reducing complexity and risk.
...
- **Governance Failures**:  
  Risks such as malicious proposals or collusion lie at the DAO layer. While outside the direct scope of this ECIP, mitigations include:
  - Proposal review periods  
  - Multi-sig proposal guards  
  - Transparent on-chain voting with quorum enforcement

- **DAO Capture Resilience**:  
  Even if the DAO governance layer is compromised, **treasury funds remain locked** unless a valid, quorum-approved proposal is executed through the correct DAO interface.

---

#### 3. Client Enforcement and Network Safety

- **Protocol Enforcement by Clients**:  
  Clients MUST enforce protocol-level redirection of the `BASEFEE` to the treasury.  
  Any client failing to implement this will diverge from consensus with the canonical Ethereum Classic chain.

- **Replay Protection**:  
  As with all hard forks, clients must implement **transaction replay protection** to safeguard against cross-fork execution.
...
- **Chain-Specific Execution**:  
  Treasury disbursement calls MUST include a **chain ID** and **proposal hash** in the payload. This prevents replay attacks across testnets, forks, or non-canonical chains.

- **Proposal-Linked Execution**:  
  All disbursements are tied to successful on-chain proposals. Without valid DAO approval, the Treasury contract will reject any attempted execution.
...
- **Independent Audits**:  
  The Olympia Treasury contract will undergo **dedicated, third-party audits** independent of the DAO governance logic.

- **Formal Verification**:  
  The contract will be analyzed using formal tools (e.g., Slither, MythX) to verify logic soundness.

- **Testnet Simulation**:  
  Full contract functionality — including `BASEFEE` redirection and DAO-triggered disbursements — will be tested on the **Mordor Testnet** ahead of mainnet deployment.

---

#### Summary

The Olympia Treasury is designed to preserve ETC's immutability, enforce strict governance-only disbursement, and minimize smart contract complexity. Its isolation from governance logic, hardened permission model, and full on-chain auditability make it robust against both technical and governance-based attacks. Thorough testnet deployment, auditing, and replay protections will ensure its readiness and resilience for mainnet activation.
