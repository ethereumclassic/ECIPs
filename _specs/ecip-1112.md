...
- The full `BASEFEE` amount from each EIP-1559 transaction **MUST** be credited to the Treasury address at the consensus layer.  
- This ECIP defines only the deposit requirement. Allocation, smoothing, or redistribution of Treasury-held `BASEFEE` — including mechanisms introduced in governance-layer ECIPs such as ECIP-1114 and ECIP-1115 — operates strictly **after** this deposit and does not modify the consensus rule that 100% of `BASEFEE` is credited to the Treasury.  
- This transfer **SHALL** occur during block finalization.  
- Clients that omit, alter, or redirect this behavior **SHALL** fork from the canonical chain.
