...
In its current state, the Etherplan Consensus Engine presents two centralization risks and a weakeness explained with their possible solutions in the following points.
...
### How can roposals be grouped as competing or complementary in a safe way?
...
There are several proposals in the ECIP pipeline that would enable interoperability between ETC and other blockchains. Such solutions are [ECIP-1049](https://ecips.ethereumclassic.org/ECIPs/ecip-1049), the integration of Keccak 256, [ECIP-1055](https://ecips.ethereumclassic.org/ECIPs/ecip-1055), Flyclient, and [ECIP-1046](https://ecips.ethereumclassic.org/ECIPs/ecip-1046), Merkle inclusion proofs to enable [NiPoPoWs](https://nipopows.com/). As several engineers and developer teams are interested in these upgrades, and they are gaining rough consensus in the ETC ecosystem, it is entirely possible for these to be integrated, making possible the implementation of this ECIP. As IOHK is the main technical supporter of the Cardano blockchain, and all blockchains will eventually need to interoperate with other networks, they could work on the upgrades needed on that network's side to be able to interoperate with ETC.
