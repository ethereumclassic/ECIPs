...
An `Accepted` ECIP may be changed back to `Draft` if flaws in the
specification are discovered and it needs to be reworked.  This differs
from moving from `Accepted` to `Rejected` because in this scenario it
is the details of the proposal which need changing, not the overall
intention of the proposal.  We are fallible humans and sometimes we
get the details wrong and need to correct our proposals.  We hope to
get things correct prior to moving any proposal to `Accepted`, but we
may not always achieve that target.  This "path" is a recognition of
that reality.
