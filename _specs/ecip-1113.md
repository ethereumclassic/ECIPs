...
This ECIP defines the on-chain governance framework for Olympia DAO, a decentralized smart contract system that governs the disbursement of funds from the Olympia Treasury (as defined in ECIP-1112). The DAO supports permissionless proposal submission, token-weighted voting, and time-locked execution of approved proposals. Its purpose is to ensure that Ethereum Classic’s protocol-level funding is allocated transparently and in alignment with the network’s principles of decentralization, neutrality, and sustainability.
...
This ECIP specifies the architecture and operational framework of Olympia DAO — a decentralized autonomous organization responsible for governing the Olympia Treasury (as defined in ECIP-1112). The DAO facilitates permissionless proposal submission, token-based community voting, and time-locked execution of approved disbursements.
...
Comparable architectures have been adopted by EVM-compatible networks such as Ronin, Celo, and Mantle, where protocol-level fee revenue is governed by DAOs. Olympia DAO adapts this model to Ethereum Classic, aligning sustainable development funding with Proof-of-Work consensus and monetary immutability.
...
- **Voting Mechanism**:  
  All proposals SHALL be subject to an on-chain vote. The initial implementation MAY support token-weighted, reputation-based, or hybrid voting models (e.g., staked ETC, veOLY, or quadratic schemes).  
  No governance token SHALL exist at launch. Any token design, supply, or distribution MUST be proposed and ratified through DAO governance, with no premine or privileged allocations.
...
Token-weighted voting systems face criticism for enabling large holders (“whales”) to dominate outcomes. Olympia DAO mitigates this risk through:

- **Modular Voting Architecture**:  
  The initial system MAY use stake-based or delegated voting. However, the DAO supports migration to alternative models, including:
...
- **Vote Delegation Mechanisms**:  
  Token holders may delegate voting rights to trusted representatives. This improves participation and lowers the barrier for non-technical users.
