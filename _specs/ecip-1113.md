...
- **Arbitrum**  
  DAO-governed treasury with an on-chain AIP process; funds ecosystem grants. Treasury is capitalized from initial token allocation and sequencer revenue, not `BASEFEE` redirection.

- **Optimism (Retroactive Public Goods Funding)**  
  Bicameral governance model (Token House + Citizens’ House) providing retroactive funding for high-impact public goods. Treasury inflows come from token supply allocation and sequencer revenue, not `BASEFEE`.
...
> *Note*: The L2 examples above (Arbitrum, Optimism, Mantle, OP Stack chains) fund treasuries via sequencer revenue or token allocations rather than `BASEFEE` redirection. They are included as governance/treasury design precedents relevant to ETC.
...
- **Arbitrum DAO (AIP Framework)**  
  Large-scale, on-chain governance managing one of the largest L2 treasuries. Uses an AIP (Arbitrum Improvement Proposal) lifecycle with community voting, Security Council oversight, and timelocked execution.

- **OlympusDAO Governance**  
  A DeFi-native treasury governance system coordinating multi-billion-dollar on-chain assets. Uses forum discussion, community signaling, and binding on-chain votes to manage protocol parameters, treasury allocations, and strategic initiatives.

- **ENS DAO Governance**  
  Controls the Ethereum Name Service protocol and treasury through on-chain proposals, delegated token voting, and a transparent proposal review process. Known for community accessibility and long-term stewardship focus.

- **MakerDAO (MIPs Framework)**  
  Manages the Dai stablecoin system and multi-billion-dollar reserves through Maker Improvement Proposals (MIPs), weekly governance cycles, and risk parameter management. Combines on-chain execution with rigorous off-chain domain team review.

- **Gitcoin DAO Governance**  
  Specializes in funding open-source software and digital public goods. Operates quadratic funding and direct grants rounds via community voting, with Gitcoin Passport providing sybil-resistance. Integrates off-chain deliberation with on-chain treasury disbursement.
