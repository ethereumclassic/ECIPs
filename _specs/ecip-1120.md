...
function calculateDistributedFees(
  ...
  for (let i = 0; i < N; i++) {
    ...
    const totalFee = ancestor.baseFee * ancestor.gasUsed;
    const baseShare = totalFee / N;
    const remainder = totalFee % N;
...
Specific parameters, including `BACKWARD_FEES_BLOCK_COUNT` and the shape of the distribution curve (linear vs decay), are to be determined by [further research](#research-plan). A number within the range of 16 and 256 blocks seems reasonable.
...
Before moving from WIP to Draft, research will be completed in roughly the following order.
