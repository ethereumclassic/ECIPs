...
This proposal activates select Ethereum protocol upgrades on Ethereum Classic via a hard fork named Olympia. Specifically, it introduces EIP-1559 and EIP-3198 to improve fee market efficiency and EVM compatibility. The goal is to enhance interoperability with the broader Ethereum ecosystem while preserving Ethereum Classic's Proof-of-Work consensus and core principles.
...
Introduces a new transaction type with a dynamically adjusting base fee per gas, which is burned each block, along with an optional tip to incentivize miners. This mechanism improves fee market efficiency, reduces volatility in gas pricing, and establishes a foundation for a sustainable block space economy on Ethereum Classic.
...
These changes provide three core benefits:
...
1. Fee Market Modernization – EIP-1559 introduces a predictable and more user-friendly gas pricing mechanism, aligning ETC with fee market improvements already adopted by major EVM chains.

1. Network Utility and Adoption – Aligning with the broader EVM roadmap ensures Ethereum Classic remains relevant for developers, tooling providers, infrastructure teams, and users seeking a Proof-of-Work alternative to Ethereum.

By narrowing the technical gap between Ethereum Classic and other EVM networks, Olympia enables ETC to remain a credible and interoperable smart contract platform while preserving its unique commitment to Proof-of-Work and protocol immutability.
...
Introduces a new transaction type with a dynamic basefee, burned per block, and optional miner tip.
