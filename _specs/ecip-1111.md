...
This proposal introduces EIP-1559 and EIP-3198 to Ethereum Classic via a hard fork named **Olympia**. These upgrades enhance EVM compatibility and improve gas pricing mechanics by adopting Ethereum’s EIP-1559 dynamic fee model. Instead of burning the `BASEFEE`, the protocol redirects it to the **Olympia Treasury** (defined in ECIP-1112), a decentralized funding contract governed by **Olympia DAO** (ECIP-1113). This provides Ethereum Classic with a sustainable, non-inflationary funding mechanism for public goods and development, while preserving Proof-of-Work consensus and core principles of immutability and decentralization.

This provides Ethereum Classic with a sustainable, non-inflationary funding mechanism for public goods and development, while preserving Proof-of-Work consensus and core principles of immutability and decentralization.
...
All `BASEFEE` revenue is redirected to the **Olympia Treasury** (ECIP-1112), a protocol-owned smart contract governed on-chain by **Olympia DAO** (ECIP-1113) and funded through the **Ethereum Classic Funding Proposal (ECFP)** process (ECIP-1114). Off-chain execution, legal compliance, and fiat interfacing are facilitated by the **Ethereum Classic DAO LLC**, a Wyoming-registered entity which operates under instructions from Olympia DAO.
...
Clients and infrastructure providers should test implementation thoroughly using test vectors and inter-client consensus tools to validate behavior across the network.

#### Monetary Policy Consideration

Redirecting the BASEFEE to the Olympia Treasury alters Ethereum's default burn behavior but does not increase ETC’s supply or introduce inflation. Ethereum Classic’s fixed monetary schedule, defined by ECIP-1017, remains fully intact. This change redirects value that would otherwise be burned into a transparent funding mechanism — preserving monetary integrity while enabling sustainable development.
...
- **Hardcoded Behavior**:  
  This redirection is enforced at the consensus layer. It cannot be bypassed, rerouted, or intercepted by external actors.
